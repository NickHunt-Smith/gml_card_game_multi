...
if eff_type = "created" && instance_exists(card_state) {
	...
	card_state.tooltip_text = "Discard";
	card_state.tooltip_text_plural = true;
	
	if opp = true {
		card_state.tooltip_counter = global.opponent.discards;
	} else {
		card_state.tooltip_counter = global.player.discards;
	}
}
...
if (eff_type = "play") && (wait = false) {
	if global.resolve_stack = true {
		...
		if eff_done = true {
			...
			if opp = true {
				restore_amount = global.opponent.discards;
			} else {
				restore_amount = global.player.discards;
			}

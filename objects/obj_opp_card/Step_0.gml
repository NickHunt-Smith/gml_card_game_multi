...
if discard_in_progress = false && discarding = true {
...
if (big_mode = false) {
	...
	x = lerp(x, 1520 + 180*(order_in_hand) -90*(array_length(global.cards_in_hand_opp) - 1), lerp_amount);
...
if mouse_y < 274 {
	y = lerp(y,y_hover,lerp_amount);
} else {
	y = lerp(y,y_unhover,lerp_amount);
}
...
if eff_type = "lingering" {
	if first_linger = true {
		rarity_memory = card_state.rarity;
		first_linger = false;
	}
	
	if card_state.rarity != rarity_memory {
		if channel_up_seq = noone {
			...
			seq_restore_change_1 = scr_change_numbers(channel,card_state.x+global.channel_offset_x,card_state.y+328);
			if card_state.frozen = true {
				card_state.memory_channel += channel;
			} else {
				card_state.channel += channel;
			}
			rarity_memory = card_state.rarity;
			alarm[1] = 90;
		}

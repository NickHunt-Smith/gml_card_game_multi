...
if eff_type = "lingering" {
	...
	var emerald_neighbour = false;
	if neighbour_1 != noone {
		if neighbour_1.element = 4 {
			emerald_neighbour = true;
		} 
	}
	if emerald_neighbour = false && neighbour_2 != noone {
		if neighbour_2.element = 4 {
			emerald_neighbour = true;
		}
	}
	
	if emerald_neighbour = true && bonus_active = false {
		fortify_seq = layer_sequence_create("effect_layer",card_state.x+112,card_state.y+230,seq_fortify)
		seq_restore_change_1 = scr_change_numbers(bonus_strength,card_state.x+global.health_offset_x,card_state.y+328);
		card_state.health_locale += bonus_strength;

		seq_restore_change_2 = scr_change_numbers(bonus_channel,card_state.x+global.channel_offset_x,card_state.y+328);
		if card_state.frozen = true {
			card_state.memory_channel += bonus_channel;
		} else {
			card_state.channel += bonus_channel;
		}
		bonus_active = true; 
		alarm[1] = 90;
	}
	
	if emerald_neighbour = false && bonus_active = true {
		fortify_seq = layer_sequence_create("effect_layer",card_state.x+112,card_state.y+230,seq_health_reduce)
		seq_restore_change_1 = scr_change_numbers(-bonus_strength,card_state.x+global.health_offset_x,card_state.y+328);
		card_state.health_locale -= bonus_strength;

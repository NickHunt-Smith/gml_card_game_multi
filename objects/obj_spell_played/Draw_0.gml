...
if big_mode = false {
	...
	if moused = true && target_arrows_enabled = true {
		depth = -900;
		if target_1 != noone && target_1 != "hand" {
			if instance_exists(target_1) && target_1.position != "end_turn" {
	...
	if variable_instance_exists(effect_function_inst,"keywords") {
		var keyword_string = "";
		for (var _i = 0; _i < array_length(effect_function_inst.keywords); _i++) {
			keyword_string += global.keyword_descriptors[$ effect_function_inst.keywords[_i]] + "\n";
		}
		draw_set_font(Empower_font);

...
if eff_type = "created" && instance_exists(card_state) {
	...
	card_state.tooltip_text = "Frozen locale";
	card_state.tooltip_text_plural = true;
	
	if opp = true {
		card_state.tooltip_counter = global.opponent.frozen_locales;
	} else {
		card_state.tooltip_counter = global.player.frozen_locales;
	}
}
...
if eff_type = "play" {
	...
	card_state.memory_channel = card_state.channel;
	card_state.channel = 0;
	if opp = true {
...
if eff_type = "lingering" {
	if ((opp = true && global.opponent.frozen_locales >= boost_threshold) or (opp = false && global.player.frozen_locales >= boost_threshold)) && boosting = false {
		...
		if card_state.frozen = true {
			card_state.memory_channel += boosted_channel;
		} else {
			card_state.channel += boosted_channel;
		}
		boosting = true;
	}
